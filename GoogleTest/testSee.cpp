...
namespace {
	...
	TEST_F(SeeTest, PawnCap) {
		...
		int move = serializeMove(H5, G4, Piece::EMPTY);
	...
	TEST_F(SeeTest, RookCap) {
		...
		int move = serializeMove(C5, C1, Piece::EMPTY);
	...
	TEST_F(SeeTest, EnPas) {
		...
		int move = serializeMove(G5, H6, Piece::EMPTY);
		EXPECT_EQ(see(pBoard, move), 0);
		EXPECT_EQ(see_ge(pBoard, move, 0), 1);
	}
	...
	TEST_F(SeeTest, StackedRooksQueen) {
		...
		int move = serializeMove(E6, E4, Piece::EMPTY);
	...
	TEST_F(SeeTest, StackedPieces) {
		...
		int move = serializeMove(H7, E4, Piece::EMPTY);
		EXPECT_EQ(see(pBoard, move), pieceScores[Piece::PAWN]);
		EXPECT_EQ(see_ge(pBoard, move, pieceScores[Piece::PAWN]), 1);
	}
