...
TEST_F(PositionalTest, CheckingMoves) {
	...
	parseFen(pBoard, "8/5k2/8/2pP4/8/Q7/8/4K3 w - c6 0 1");
    move = serializeMove(E1, D1, NORMAL_MOVE, Pieces::NO_PIECE);
	EXPECT_FALSE(checkingMove(pBoard, move));
...
TEST_F(PositionalTest, DiscoveredAttacks) {
    ...
    discoveredAtks = getDiscoveredAttacks(pBoard, A1, chai::BLACK);
	EXPECT_FALSE(discoveredAtks);
    discoveredAtks = getDiscoveredAttacks(pBoard, C7, chai::BLACK);
	EXPECT_FALSE(discoveredAtks);
...
TEST_F(PositionalTest, Pinner) {
    ...
    pinner = getPinner(pBoard, H8, chai::BLACK);
    pinned = getPinned(pBoard, H8, chai::BLACK);
    ...
    pinner = getPinner(pBoard, G7, chai::BLACK);
    pinned = getPinned(pBoard, G7, chai::BLACK);
	EXPECT_TRUE(pinner & (1ULL << D4));
